...
const Dashboard = () => {
  ...
  const totalEnheder = pakker.reduce((sum, p) => sum + (p.data.enheder || 0), 0);
  
  // Calculate total actual usage (accumulated + current meter)
  ...
  sortedPakker.forEach(pakke => {
    const pakkeEnheder = parseFloat(pakke.data.enheder);
        ...
        {isCheckedIn && pakker.length > 0 && (
          <div className="space-y-4 animate-fade-in">
            ...
            {sortedPakker.map((pakke, index) => {
              ...
              const remaining = Math.max(0, pakke.data.enheder - usage);
